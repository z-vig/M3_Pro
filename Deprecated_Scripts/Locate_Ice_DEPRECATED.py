...
class HDR_Image():
    # Constructor method
    def __init__(self,image_path,location_path):
        self.image_hdr = sp.envi.open(image_path)
        self.location_hdr = sp.envi.open(location_path)
        image_data_str = self.image_hdr.filename
        location_data_str = self.location_hdr.filename
        dateTimeIndex = find(image_data_str,'t')[-1]

        date = f"{image_data_str[dateTimeIndex-8:dateTimeIndex-4]}-{image_data_str[dateTimeIndex-4:dateTimeIndex-2]}-{image_data_str[dateTimeIndex-2:dateTimeIndex]}"

        time = f"{image_data_str[dateTimeIndex+1:dateTimeIndex+3]}-{image_data_str[dateTimeIndex+3:dateTimeIndex+5]}-{image_data_str[dateTimeIndex+5:dateTimeIndex+7]}"

        date_time = date+'_'+time

        if image_data_str[dateTimeIndex-9] == 'g':
            obs_type = 'Global'
        elif image_data_str[dateTimeIndex-9] == 't':
            obs_type = 'Target'
        else:
            print (f'HDR File Name: {self.image_hdr.filename}')
            print (find(self.image_hdr.filename, '/'))
            print (image_data_str)
            print (f'Observation Type loaded as: {image_data_str[2]}')
            raise Exception("Data String Error!")
        
        image_dataType = image_data_str[len(image_data_str)-7:len(image_data_str)-4]
        location_dataType = location_data_str[len(location_data_str)-7:len(location_data_str)-4]

        if image_dataType == 'sup':
            data_type = "Supplemental"
        elif image_dataType == 'rfl':
            data_type = "Reflectance"
        else:
            pass

        if location_dataType == 'loc':
            data_type = "Location"
        elif location_dataType == 'obs':
            data_type = "Observation"
        elif location_dataType == 'rdn':
            data_type = 'Radiance'
        else:
            data_type = "Cannot Parse Location"
        ...
        if self._dataType == 'Location' or self._dataType == 'Observation' or self._dataType == 'Radiance':
            self.bandCenters = None
        else:
            self.bandCenters = np.array(self.image_hdr.bands.centers)
            self.allowedIndices = np.where((self.bandCenters>900)&(self.bandCenters<2600))[0]
        
    # String print method
    def __str__(self):
        return f"HDR Image Class: {self.image_hdr.fid.name[find(self.image_hdr.fid.name,'/')[-1]+1:len(self.image_hdr.fid.name)-4]}"

    # Returns HDR Object for testing purposes
    def hdr(self):
        return self.image_hdr
    ...
    @property
    def bandnames(self):
        return self.image_hdr.__dict__.get('metadata').get('band names')
    ...
    def get_lat_long(self):
        if self.datatype == 'Location':
            self.long_band = self.location_hdr.read_band(0)
            self.lat_band = self.location_hdr.read_band(1)
            self.elevation = self.location_hdr.read_band(2)
        return self.long_band,self.lat_band,self.elevation
    
    def get_shadow_coords(self,**kwargs):
        defaultKwargs = {}
        kwargs = {**defaultKwargs,**kwargs}

        if self.datatype == 'Observation':
            solar_incidence_index = self.location_hdr.bandnames.index('To-Sun Zenith (deg)')
            solar_incidence_image = self.location_hdr.read_band(solar_incidence_index)
            shadow_image = copy(solar_incidence_image)
            shadow_image[np.where(solar_incidence_image<90)] = 85
            fig = plt.figure()
            plt.imshow(shadow_image)
            print (np.max(solar_incidence_image))

            xShadow,yShadow = (np.where(solar_incidence_image<90))
            return xShadow,yShadow
    ...
    def original_image(self,**kwargs):
        ...
        self.originalImage = self.image_hdr.read_bands(self.allowedIndices)
...
#%%
image_FileList,image_FilesPath = M3_UnZip.M3_unzip(select=True)
location_FileList,location_FilesPath = M3_UnZip.M3_unzip(select=True)
#%%
for imageFile,locFile in zip(image_FileList,location_FileList):
    stamp = HDR_Image(imageFile,locFile)
    print (stamp.datatype)

#%%
for imageFile,locFile in zip(image_FileList,location_FileList):
    loc_dict = {}
    img = HDR_Image(imageFile,locFile)
    if img.datatype == 'Location':
        long,lat,elev = img.get_lat_long()
        ...
        loc_dict.update({'Latitude':lat.flatten(),\
                         'Longitude':long.flatten(),\
                         'Elevation':elev.flatten()})
        loc_df = pd.DataFrame(loc_dict)
        break
        #loc_df.to_csv(f'D:/Data/Ice_Pipeline_Out_4-13-23/locationInfo/{img.datetime}.csv')
...
for imagefile,locFile in zip(image_FileList,location_FileList):
    img = HDR_Image(imageFile,locFile)
    if img.datatype == 'Radiance':
        wvl = np.array(img.hdr.bands.centers)
        allowed_Indices = np.where((wvl>900)&(wvl<2600))[0]
        allowedWvl = wvl[allowed_Indices]
