'''
Script for utilizing the M3 Image Class to smooth hyperspectral images over the wavelength dimension
'''
...
def spectrum_smoothing(save_step:bool)->None:
    print ('Select Analysis Folder:')
    folder_path = askdir()
    print (f'{folder_path} selected for analysis')
    print ('Select input folder for spectrum smoothing step:')
    input_path = askdir()
    ...
    for input_path,loc_path,obs_path,stamp_name in zip(all_input_paths,all_loc_paths,all_obs_paths,stamp_names):
        ...
        stamp_object.spectrum_smoothing(save_step=save_step)
        prog+=1
        print (f'\rAnalysis for {stamp_name} complete ({prog/tot:.2%})',end='\r')

if __name__ == "__main__":
    ##Running cubic split fit smooth step
